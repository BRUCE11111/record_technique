...
![](进程间的通信IPC/进程使用二值信号量.PNG)

两个进程使用一个二值信号量

![](进程间的通信IPC/进程使用Posix有名二值信号量.PNG)

两个进程所以用一个Posix有名二值信号量

![](进程间的通信IPC/一个进程两个线程共享基于内存的信号量.PNG)
